...
struct sdmmc_request {
    ...
    void* virt;
    // required for DMA
    phys_iter_buffer_t phys;

    // response data
    uint32_t response[4];

    // completion callback. it is illegal to call request() from this callback
    void (*complete_cb)(sdmmc_request_t* req, void* cookie);
    void* cookie;
...
static inline zx_status_t sdmmc_request(sdmmc_protocol_t* sdmmc, sdmmc_request_t* req) {
