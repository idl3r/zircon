...
#define MMC_RCA_ARG(dev) ((dev)->rca << 16)
...
static zx_status_t mmc_send_op_cond(sdmmc_device_t* dev, uint32_t ocr, uint32_t* rocr) {
    ...
    zx_status_t st;
    for (int i = 100; i; i--) {
        if ((st = sdmmc_do_request(dev, &req)) != ZX_OK) {
...
static zx_status_t mmc_all_send_cid(sdmmc_device_t* dev, uint32_t cid[4]) {
    sdmmc_request_t req = {
        .cmd = SDMMC_ALL_SEND_CID,
        .arg = 0,
    };
    zx_status_t st = sdmmc_do_request(dev, &req);
    if (st == ZX_OK) {
...
static zx_status_t mmc_set_relative_addr(sdmmc_device_t* dev) {
    sdmmc_request_t req = {
...
static zx_status_t mmc_send_csd(sdmmc_device_t* dev, uint32_t csd[4]) {
    sdmmc_request_t req = {
        .cmd = SDMMC_SEND_CSD,
        .arg = MMC_RCA_ARG(dev),
    };
...
static zx_status_t mmc_send_ext_csd(sdmmc_device_t* dev, uint8_t ext_csd[512]) {
...
static zx_status_t mmc_select_card(sdmmc_device_t* dev) {
    sdmmc_request_t req = {
        .cmd = MMC_SELECT_CARD,
        .arg = MMC_RCA_ARG(dev),
    };
...
static zx_status_t mmc_switch(sdmmc_device_t* dev, uint8_t index, uint8_t value) {
    ...
    sdmmc_request_t req2 = {
        .cmd = SDMMC_SEND_STATUS,
        .arg = MMC_RCA_ARG(dev),
    };
    st = sdmmc_do_request(dev, &req2);
    if (st == ZX_OK) {
        if (req2.response[0] & MMC_STATUS_SWITCH_ERR) {
...
static zx_status_t mmc_set_bus_width(sdmmc_device_t* dev, sdmmc_bus_width_t bus_width,
    ...
    zx_status_t st = mmc_switch(dev, MMC_EXT_CSD_BUS_WIDTH, mmc_ext_csd_bus_width);
    if (st != ZX_OK) {
...
static zx_status_t mmc_switch_timing(sdmmc_device_t* dev, sdmmc_timing_t new_timing) {
    ...
    zx_status_t st = mmc_switch(dev, MMC_EXT_CSD_HS_TIMING, ext_csd_timing);
    if (st != ZX_OK) {
...
zx_status_t sdmmc_probe_mmc(sdmmc_device_t* dev) {
    ...
    dev->rca = 1;
    if ((st = mmc_set_relative_addr(dev)) != ZX_OK) {
