...
typedef struct sdhci_device {
    ...
    sdmmc_request_t* cmd_req;
    // Current data line request
    sdmmc_request_t* data_req;
...
static void sdhci_complete_request_locked(sdhci_device_t* dev, sdmmc_request_t* req,
                                          zx_status_t status) {
    zxlogf(TRACE, "sdhci: complete cmd 0x%08x\n", req->cmd);
...
static void sdhci_cmd_stage_complete_locked(sdhci_device_t* dev) {
    ...
    sdmmc_request_t* req = dev->cmd_req;
...
static void sdhci_data_stage_read_ready_locked(sdhci_device_t* dev) {
    ...
    sdmmc_request_t* req = dev->data_req;
...
static zx_status_t sdhci_build_dma_desc(sdhci_device_t* dev, sdmmc_request_t* req) {
    phys_iter_t iter;
    phys_iter_init(&iter, &req->phys, ADMA2_DESC_MAX_LENGTH);
...
static zx_status_t sdhci_start_req_locked(sdhci_device_t* dev, sdmmc_request_t* req) {
    ...
    if (has_data && !sdmmc_cmd_rsp_busy(cmd)) {
        if ((req->use_dma && !req->phys.phys) && !req->virt && (cmd != MMC_SEND_TUNING_BLOCK)) {
            zxlogf(TRACE, "sdhci: cmd 0x%08x invalid dma %d bsy %d phys %p virt %p\n",
                   cmd, req->use_dma, sdmmc_cmd_rsp_busy(cmd), req->phys.phys, req->virt);
...
static void sdhci_tuning_cplt(sdmmc_request_t* req, void* cookie) {
...
static zx_status_t sdhci_perform_tuning(void* ctx) {
    ...
    int count = 0;
    do {
        sdhci_start_req_locked(dev, &req);
...
static zx_status_t sdhci_controller_init(sdhci_device_t* dev) {
    ...
    if (sdhci_supports_adma2_64bit(dev)) {
        ...
        dev->info.max_transfer_size = DMA_DESC_COUNT * ADMA2_DESC_MAX_LENGTH;
